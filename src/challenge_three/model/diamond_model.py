...
directory_model_evaluation = os.path.join('src', 'challenge_three', 'model', 'model_weights')
...
def load_metrics():
    ...
    if files:
        ...
        # Read the file and extract RMSE, MAE, and R-squared values
        with open(newest_model_file, 'r') as file:
            lines = file.readlines()
            rmse = float(lines[2].split(': ')[1])
            mae = float(lines[3].split(': ')[1])
            r_squared = float(lines[4].split(': ')[1])
...
def load_model() -> Optional[xgb.XGBRegressor]:
    ...
    """
    global directory_model_evaluation
...
def train(xgb_regressor: Optional[xgb.XGBRegressor] = None,
          new_data_to_train: Optional[pd.DataFrame] = None,
          retrain: Optional[bool] = False) -> (
        Tuple[Optional[xgb.XGBRegressor], Tuple[float, float, float]]
):
    """
    ...
    """
    global X_test_original, y_test_original, dataset_directory
    ...
    return xgb_regressor, (xgb_rmse, xgb_mae, xgb_r2)
